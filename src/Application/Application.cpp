...
void Application::ParseExtraParams(size_t const StartIndex)
{
	for (size_t i = StartIndex; i < commandArguments.size(); ++ i)
	{
		...
		}else if (argument == "-softs")
		{
			params.useSS = true;
		}else if (StringBeginsWith(argument, "-softs=", remainder))
		{
         int num = std::stoi(remainder);
         if(num >=16){
            params.shadowSamples = num;
         }else{
            params.shadowSamples = 16;
         }
			params.shadowSamples = std::stoi(remainder);
		}else if (StringBeginsWith(argument, "-radius=", remainder))
		{
         params.shadowRadius = std::stoi(remainder);
		}
	}
}
